...
try:
    if os.path.exists("firebase-adminsdk.json"):
        ...
        print("Inicializando Firebase con firebase-adminsdk.json")
    else:
        required_env = [
...
def save_json(data):
    try:
        ...
        doc_ref.set(data, merge=False)  # Sobrescribir completamente el documento
        print(f"Datos guardados en Firestore bajo 'default_user': {data}")
        return True
    except Exception as e:
        print(f"Error al guardar en Firestore: {str(e)} - Datos intentados: {data}")
        return False
...
def get_json():
    try:
        ...
        if doc_ref.exists:
            result = doc_ref.to_dict()
            print(f"Datos obtenidos de Firestore: {result}")
            return [result]
        print("Documento 'default_user' no encontrado en Firestore")
        return []
    except Exception as e:
