...
def datos_personales():
    ...
    if request.method == 'POST':
        try:
            ...
            velocidad_crecimiento_actual = datos_grafica[-1]['velocidad'] * 100 if datos_grafica else 0
...
# El resto de las rutas (nuevo_usuario, entreno, etc.) no necesitan cambios
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if accion == 'registrar':
                ...
                km_semanal = sum(float(k) for f, k in modelo.km_corridos.items() if inicio_semana.strftime('%Y-%m-%d') <= f <= fin_semana.strftime('%Y-%m-%d'))
                tiempo_semanal = sum(float(t) for f, t in modelo.tiempo_corridos.items() if inicio_semana.strftime('%Y-%m-%d') <= f <= fin_semana.strftime('%Y-%m-%d'))
