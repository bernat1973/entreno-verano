...
def _get_datos_personales_context(extra_context={}):
    """
    Centraliza la obtención de todo el contexto necesario para la plantilla de datos personales.
    """
    ...
    context = {
        ...
        'peso': modelo.peso,
        'estatura': modelo.estatura * 100 if modelo.estatura else 0,
        'talla_sentada': modelo.talla_sentada * 100 if modelo.talla_sentada else 0,
        'envergadura': modelo.envergadura * 100 if modelo.envergadura else 0,
        'meta_km': modelo.meta_km.get(semana_ano, 0),
        'ejercicios_type': modelo.ejercicios_type,
        'mes_medicion': date.today().strftime('%Y-%m')
    }
    
    context['segmento_inferior'] = (modelo.estatura - modelo.talla_sentada) * 100 if modelo.estatura and modelo.talla_sentada else 0
    context['imc'] = modelo.peso / (modelo.estatura ** 2) if modelo.estatura and modelo.peso else 0
    context['datos_grafica'] = _calcular_datos_grafica(modelo.historial_mediciones)
    context['velocidad_crecimiento'] = context['datos_grafica'][-1]['velocidad'] if context['datos_grafica'] else 0
...
def datos_personales():
    if request.method == 'POST':
        try:
            ...
            if not nombre:
                return render_template('datos_personales.html', **_get_datos_personales_context({'error': "El nombre no puede estar vacío."}))
            ...
            if any(v < 0 for v in [peso, estatura, talla_sentada, envergadura, meta_km]):
                return render_template('datos_personales.html', **_get_datos_personales_context({'error': "Los valores deben ser positivos."}))
            ...
            modelo.ejercicios_type = request.form.get('ejercicios_type', 'bodyweight')
            ...
            context = _get_datos_personales_context({'mensaje': "¡Datos guardados correctamente!"})
            return render_template('datos_personales.html', **context)
        
        except (ValueError, Exception) as e:
            error_msg = f"Error en los datos de entrada: {e}" if isinstance(e, ValueError) else f"Error interno: {e}"
            return render_template('datos_personales.html', **_get_datos_personales_context({'error': error_msg}))

    # GET request
    return render_template('datos_personales.html', **_get_datos_personales_context())

@app.route('/cambiar_usuario', methods=['POST'])
def cambiar_usuario():
    try:
        nombre_usuario = request.form['usuario'].strip()
        if not nombre_usuario or nombre_usuario not in modelo.get_usuarios():
            return render_template('datos_personales.html', **_get_datos_personales_context({'error': "Selecciona un usuario válido."}))
        
        if modelo.cambiar_usuario(nombre_usuario):
            context = _get_datos_personales_context({'mensaje': f"Cambiado a usuario '{nombre_usuario}'!"})
            return render_template('datos_personales.html', **context)
        else:
            return render_template('datos_personales.html', **_get_datos_personales_context({'error': f"Error al cambiar a usuario '{nombre_usuario}'."}))

    except Exception as e:
        return render_template('datos_personales.html', **_get_datos_personales_context({'error': f"Error interno al cambiar de usuario: {e}"}))
...
def nuevo_usuario():
    try:
        ...
        if not nuevo_nombre:
            return render_template('datos_personales.html', **_get_datos_personales_context({'error': "El nombre del nuevo usuario no puede estar vacío."}))
        if nuevo_nombre in modelo.get_usuarios():
            return render_template('datos_personales.html', **_get_datos_personales_context({'error': "El usuario ya existe."}))
        
        modelo.nuevo_usuario(nuevo_nombre)
        # Después de crear, cambiamos a ese usuario para mostrar sus datos (que estarán en blanco)
        modelo.cambiar_usuario(nuevo_nombre)
        
        context = _get_datos_personales_context({'mensaje': f"¡Usuario '{nuevo_nombre}' creado y seleccionado correctamente!"})
        return render_template('datos_personales.html', **context)

    except Exception as e:
        return render_template('datos_personales.html', **_get_datos_personales_context({'error': f"Error interno al crear usuario: {e}"}))
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if accion == 'registrar':
                ...
                modelo.registrar_corrida(fecha, km, tiempo_segundos)
